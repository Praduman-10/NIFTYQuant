...
.block-container { max-width:1700px; padding-top:2.2rem; }
...
.nq-kicker { color:#777; font-size:.7rem; font-weight:800; letter-spacing:1.7px; }
...
if vrp >= 2:
    regime = "PREMIUM SELL / CAUTION"
elif vrp <= -2:
    regime = "LONG VOL BIAS"
elif pcr >= 1.2:
    regime = "PUT-SIDE SUPPORT"
elif pcr <= 0.8:
    regime = "CALL-SIDE PRESSURE"
else:
    regime = "NEUTRAL"
...
# KPI row
kpis = [
    ("NIFTY SPOT", f"{spot:,.2f}", f"ATM {atm:,.0f}"),
    ("ATM IV", f"{atm_iv:.2f}%", "Implied volatility"),
    ("REALIZED HV", f"{hv:.2f}%", "20D historical volatility"),
    ("PCR", f"{pcr:.2f}", f"PE OI {fmt_compact(pe_oi)}"),
    ("VRP", f"{vrp:+.2f}%", "IV minus HV"),
]
cols = st.columns(5)
...
def render_trade(row, score_col, side):
    score = float(row[score_col]); q = float(row["trade_quality"])
    cls = "green" if side == "BUY" else "red"
...
read_text = (
    "Options are trading above realized volatility, indicating a positive volatility risk premium in the current snapshot."
    if vrp > 0 else
    "Options are trading below realized volatility, indicating a negative volatility risk premium in the current snapshot."
    if vrp < 0 else
    "Implied and realized volatility are approximately aligned in the current snapshot."
)
mc1, mc2 = st.columns([2, 1])
with mc1:
    st.markdown(f'<div style="font-size:1.15rem;font-weight:900;color:#fff;">{regime}</div><div style="color:#888;font-size:.75rem;line-height:1.6;margin-top:7px;">{read_text}</div>', unsafe_allow_html=True)
with mc2:
    st.markdown(f'<div style="color:#777;font-size:.7rem;">SPOT / ATM</div><div style="color:#fff;font-weight:900;margin-top:3px;">{spot:,.2f} / {atm:,.0f}</div><div style="color:#777;font-size:.7rem;margin-top:8px;">PCR / VRP</div><div style="color:#fff;font-weight:900;margin-top:3px;">{pcr:.2f} / {vrp:+.2f}%</div>', unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
